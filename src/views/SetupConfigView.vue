...
<template>
  <div>
    ...
    <div v-if="activeTab === 'servers'" class="d-flex flex-column gap-4 mb-4">
      ...
      >
        <div class="card-body px-3 py-3">
          ...
          <div class="row g-2">
            <div class="col-sm-6">
              <label class="form-label small fw-medium mb-1">{{ t('setup.authUrl') }}</label>
              <input v-model="server.authUrl" type="url" class="form-control form-control-sm" :placeholder="t('setup.authUrlPlaceholder')" />
            </div>
            <div class="col-sm-6">
              <label class="form-label small fw-medium mb-1">{{ t('setup.settingsUrl') }}</label>
              <input v-model="server.settings" type="url" class="form-control form-control-sm" :placeholder="t('setup.settingsUrlPlaceholder')" />
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-else class="mb-4">
      ...
      <div v-for="(mod, index) in mods" :key="index" class="card border-0 mb-2" style="background: var(--bs-tertiary-bg);">
        <div class="card-body px-3 py-2">
          <div class="row g-2 align-items-center">
            <div class="col-sm-3"><input v-model="mod.name" class="form-control form-control-sm" :placeholder="t('setup.modName')" /></div>
            <div class="col-sm-3"><input v-model="mod.fileName" class="form-control form-control-sm" :placeholder="t('setup.modFileName')" /></div>
            <div class="col-sm-3"><input v-model="mod.description" class="form-control form-control-sm" :placeholder="t('setup.modDescription')" /></div>
            <div class="col-sm-2 d-flex gap-3 align-items-center">
              <div class="form-check form-check-inline mb-0">
                <input v-model="mod.recommended" class="form-check-input" type="checkbox" :id="`mod-rec-${index}`" />
                <label class="form-check-label small" :for="`mod-rec-${index}`">{{ t('setup.modRecommended') }}</label>
              </div>
              <div class="form-check form-check-inline mb-0">
                <input v-model="mod.enabled" class="form-check-input" type="checkbox" :id="`mod-en-${index}`" />
                <label class="form-check-label small" :for="`mod-en-${index}`">{{ t('setup.modEnabled') }}</label>
              </div>
            </div>
            <div class="col-sm-1 text-end">
              <button class="btn btn-link btn-sm text-danger p-0" @click="removeMod(index)" :aria-label="t('setup.removeMod')"><BIconTrash aria-hidden="true" /></button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="d-flex justify-content-end">
      <button class="btn btn-primary rounded-pill px-4 d-flex align-items-center gap-2" :disabled="saving" @click="saveAndContinue">
