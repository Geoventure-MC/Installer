...
function phpBackendMock(): Plugin {
  ...
  const mockData = {
    installerVersion: '1.2.0', minPhpVersion: '8.2', phpVersion: '8.2', phpFullVersion: '8.2.26',
    phpIniPath: '/etc/php/8.2/apache2/php.ini', path: '/var/www/html', file: '/var/www/html/index.php',
    htaccess: true, webConfig: false, windows: false, requirements, compatible: true,
    downloaded: false, extracted: false, latestInstallerVersion: null,
  }
  ...
  const mockNotifications = [{ id: 1, type: 'info', message: 'Bienvenue sur CentralCorp Panel !', url: null, expiresAt: null, createdAt: new Date().toISOString() }]
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use((req: any, res: any, next: any) => {
        ...
        if (action === 'api-schema') { res.end(JSON.stringify({ schemaVersion: '1.0.0' })); return }
        if (action === 'launcher-config') { res.end(JSON.stringify({ panelUrl: 'http://localhost:5173/', generatedAt: new Date().toISOString(), servers: Object.entries(mockAuthConfig).map(([id, s]) => ({ id, ...s })) })); return }
        ...
        if (req.method === 'POST') {
          readBody(req).then((body) => {
            ...
            if (postAction === 'auth-config') { Object.assign(mockAuthConfig, (body.data as Record<string, Record<string, string>>) ?? {}); res.end(JSON.stringify({ saved: true })); return }
