...
# ── Contexte lié au Launcher ───────────────────────────────────────────────────
snapshot "## Relation avec le Launcher"
snapshot "Ce projet installe le backend CentralCorp Panel."
snapshot "Le Launcher consomme : {settings}/utils/api"
snapshot "Launcher settings URL : https://launcher.bmeouchi.fr/"
snapshot "Launcher env (NE PAS CHANGER) : panel"
snapshot ""
...
if [ "$FULL" = true ]; then
  ...
  if [ -f backend/index.php ]; then
    snapshot "## backend/index.php (extrait)"
    snapshot "\`\`\`php"
    snapshot "$(head -60 backend/index.php)"
    snapshot "\`\`\`"
    snapshot ""
  fi
